...
def run_test_polygon():
    ...
    # ------------------------------------------------------------------
    title = 'POLYGON test 3:  4 segments with black lines on a green circle'
    window = rg.RoseWindow(550, 400, title)
...
def run_test_fancy_polygon():
    ...
    title = 'FANCY POLYGON test 4:  2  lines on blue circle, hops = 1. Latvian flag'
    window = rg.RoseWindow(480, 350, title)

    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'maroon'
    fancy_polygon(window, circle, 2, 1, 'white', 50)
    window.close_on_mouse_click()
